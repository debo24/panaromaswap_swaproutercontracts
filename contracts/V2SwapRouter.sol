...
/// @title Panaromaswap V2 Swap Router
/// @notice Router for stateless execution of swaps against Panaromaswap V2
abstract contract V2SwapRouter is IV2SwapRouter, ImmutableState, PeripheryPaymentsWithFeeExtended {
    ...
    function _swap(address[] memory path, address _to) private {
        for (uint256 i; i < path.length - 1; i++) {
            ...
            IPanaromaswapV2Pair pair = IPanaromaswapV2Pair(PanaromaswapV2Library.pairFor(factoryV2, input, output));
            uint256 amountInput;
            ...
            address to = i < path.length - 2 ? PanaromaswapV2Library.pairFor(factoryV2, output, path[i + 2]) : _to;
    ...
    function swapExactTokensForTokens(
        ...
        pay(
            ...
            PanaromaswapV2Library.pairFor(factoryV2, path[0], path[1]),
            amountIn
        );
    ...
    function swapTokensForExactTokens(
        ...
        amountIn = PanaromaswapV2Library.getAmountsIn(factoryV2, amountOut, path)[0];
        require(amountIn <= amountInMax, 'Too much requested');

        pay(path[0], msg.sender, PanaromaswapV2Library.pairFor(factoryV2, path[0], path[1]), amountIn);
